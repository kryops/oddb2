<?php
/**
 * pages/inva/fremd.php
 * Fremdaktionen und Kolonisationen (Inhalt)
 */
...
$content =& $csw->data['fremd']['content'];

$content = '
	<div class="hl2">Fremdaktionen und Kolonisationen</div>';

// Bedingungen aufstellen
$conds = array(
	"(invasionenEnde = 0 OR invasionenEnde > ".time().")",
	"(invasionenFremd = 1 OR invasionenTyp = 5)"
);
...
<div class="center">
<br /><br />
<form action="#" name="sort_inva" onsubmit="$(this).children(\'.link\').data(\'link\', \'index.php?p=inva&amp;sp=fremd&amp;esort=\'+this.start.value).trigger(\'click\');return false">
...
<tr>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd&amp;sort=id">Gala</a></th>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd&amp;sort=id">System</a></th>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd&amp;sort=id">Planet</a></th>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd&amp;sort=gate">Gate <span class="small">(A'.$user->settings['antrieb'].')</span></a></th>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd&amp;sort=typ">Typ</a></th>
<th>Opfer</th>
<th>Aggressor</th>
<th><a class="link" data-link="index.php?p=inva&amp;sp=fremd">Ende</a></th>
...
$content2 .= '
	...
	while($row = mysql_fetch_assoc($query)) {
		...
		if($row['invasionenTyp'] == 5 AND $row['a_player_allianzenID'] != NULL) {
			if(!isset($allies[$row['a_player_allianzenID']])) {
				$allies[$row['a_player_allianzenID']] = array($row['a_allianzenTag'], 1);
			}
			else {
				$allies[$row['a_player_allianzenID']][1]++;
			}
		}
		else if($row['player_allianzenID'] != NULL) {
			if(!isset($allies[$row['player_allianzenID']])) {
...
// Log-Eintrag
if($config['logging'] == 3) {
	insertlog(5, 'lässt sich die Liste der Fremdinvasionen anzeigen');
}
